...
public class NUnitNetCore31Driver : IFrameworkDriver
{
    ...
    private static readonly ILogger log = InternalTrace.GetLogger(nameof(NUnitNetCore31Driver));
    ...
    public string Load(string assemblyPath, IDictionary<string, object> settings)
    {
        ...
        catch (Exception e)
        {
            ...
            log.Error(msg);
            throw new BetaEngineException(msg, e);
        }

        log.Debug($"Loaded {assemblyPath}");

        var nunitRef = _testAssembly.GetReferencedAssemblies().FirstOrDefault(reference =>
            reference.Name.Equals("nunit.framework", StringComparison.OrdinalIgnoreCase));
        if (nunitRef == null)
        {
        ...
        {
            log.Error($"{FAILED_TO_LOAD_NUNIT}\r\n{e}");
            throw new BetaEngineException(FAILED_TO_LOAD_NUNIT, e);
        }
