...
public class NUnitNetStandardDriver : IFrameworkDriver
{
    ...
    private const string INVALID_FRAMEWORK_MESSAGE =
        "Running tests against this version of the framework using this driver is not supported. Please update NUnit.Framework to the latest version.";

    private const string FAILED_TO_LOAD_TEST_ASSEMBLY = "Failed to load the test assembly {0}";
    ...
    private static readonly ILogger log = InternalTrace.GetLogger(nameof(NUnitNetStandardDriver));
    ...
    public string Load(string testAssembly, IDictionary<string, object> settings)
    {
        ...
        using (var assemblyRef = AssemblyDefinition.ReadAssembly(testAssembly))
        {
            ...
            {
                throw new BetaEngineException(string.Format(FAILED_TO_LOAD_TEST_ASSEMBLY, assemblyRef.FullName));
            }

            var nunitRef = assemblyRef.MainModule.AssemblyReferences.Where(reference =>
                reference.Name.Equals("nunit.framework", StringComparison.OrdinalIgnoreCase)).FirstOrDefault();
            if (nunitRef == null)
            {
                throw new BetaEngineException(FAILED_TO_LOAD_NUNIT);
            }
            ...
            {
                throw new BetaEngineException(INVALID_FRAMEWORK_MESSAGE);
            }
    ...
    {
        CheckLoadWasCalled();
        var count = ExecuteMethod(COUNT_METHOD, filter);
